      ...
      _export("ResponsiveView", ResponsiveView = (_dec = ccclass('ResponsiveView'), _dec2 = property({
        ...
        constructor() {
          ...
          this.onResize = () => {
            ...
            var designAspect = this.designWidth / this.designHeight;
            var screenAspect = frame.height > 0 ? frame.width / frame.height : designAspect; // FIXED_HEIGHT sẽ "crop ngang" khi màn hình quá hẹp (mobile dọc siêu dài).
            // FIXED_WIDTH sẽ "crop dọc" khi màn hình quá rộng.
            // Chọn policy theo tỉ lệ để tránh crop UI (ưu tiên không cắt nội dung).

            var policy = screenAspect < designAspect ? ResolutionPolicy.FIXED_WIDTH : ResolutionPolicy.FIXED_HEIGHT;
        ...
        applyCameraViewport() {
          ...
          var screenAspect = frame.width / frame.height;
          var designAspect = this.designWidth / this.designHeight; // Letterbox chỉ khi màn hình quá rộng (thường là web landscape).

          if (screenAspect > designAspect) {
            // Màn hình rộng: giữ full height, thu hẹp width và căn giữa (2 viền đen hai bên).
            var w = designAspect / screenAspect;
            var x = (1 - w) * 0.5;
